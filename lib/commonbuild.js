...
function checkArchitecture(emitter, options, callback) {
    ...
    return callback(new Error('Unsupported architecture: '+ process.arch));
}
...
function prepareSources(emitter, options, callback) {
    ...
    if (options.type == 'rpm') {
        ...
        options.sourcesDir = path.join(options.BUILD_DIR, 'package');
    }
    ...
    async.series([
        function (callback) {
            ...
            command = 'npm pack '+ options.package;
            ...
            if (options.type == 'rpm') {
                ...
                srcdir = builddir = options.BUILD_DIR;
            }
            ...
            exec(command, { cwd: options.sourcesDir, env: { NODE_ENV: 'production', HOME: process.env.HOME } }, callback);
        }
    ], callback);
}
    ...
    async.series([
        function (callback) {
            ...
            files = files.map(function (file) {
                file = ld.isPlainObject(file) ? file : {
                    files: file,
                    target: options.installTarget,
                    cwd: ''
                };
                file.include = file.files ? (file.files[0] !== '!') : null;
                return file;
            });
            ...
            options.emptyDirs = ld.where(patterns, { empty: true });
            patterns = ld.difference(patterns, options.emptyDirs);
            ...
            async.eachSeries(options.emptyDirs, function (file, callback) {
                mkdirp(path.join(options.BUILDROOT_DIR, file.target), callback);
            }, dz(callback));
        }, function (callback) {
            ...
            targetFiles = options.emptyDirs.map(function (file) {
                return {
...
function prepareServiceFile(emitter, options, callback) {
    ...
    if ( ! prepareService) {
        return callback(new Error('Not supported service type '+ _service.type));
    }
