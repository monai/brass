...
function prepareServiceFile(callback) {
    ...
    _service = options.service;
    if ( ! _service) return dz(callback)(null);
...
function prepareDebianDirectory(callback) {
    var debian, files, dest;
    
    emitter.info('Preapring debian directory', { em: true });
    
    debian = path.join(options.BUILDROOT_DIR, 'debian');
    files = [
        'control',
        'copyright',
        'changelog',
    ...
    async.series([
        function (callback) {
            mkdirp(debian, callback);
        }, function (callback) {
            async.eachSeries(files, function (file, callback) {
                dest = path.join(debian, file);
            ...
            fs.chmod(path.join(debian, 'rules'), '744', callback);
        }
    ], dz(callback));
}
