...
function rpmbuild(_emitter, _config, callback) {
    emitter = _emitter;
    config = _config;
...
function prepareDaemonFile(callback) {
    ...
    async.series([
        function (callback) {
            ...
            putil.renderAssetFile('service/sysv', initScriptTarget, config, callback);
        }, function (callback) {
...
function prepareSpecFile(callback) {
    putil.renderAssetFile('rpmbuild/spec', path.join(config.RPMBUILD_DIR, 'SPECS', config.specfile), config, callback);
}
